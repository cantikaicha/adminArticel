...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_to_watchlist'])) {
    ...
    // Cek apakah artikel sudah ada di daftar favorite
    $check_sql = "SELECT * FROM favorite WHERE username = ? AND article_id = ?";
    ...
    if ($result->num_rows > 0) {
        echo "<script>alert('Artikel sudah ada di daftar favorite Anda.');</script>";
    } else {
        $insert_sql = "INSERT INTO favorite (username, article_id) VALUES (?, ?)";
        $stmt = $conn->prepare($insert_sql);
        $stmt->bind_param("si", $username, $article_id);
        if ($stmt->execute()) {
            echo "<script>alert('Artikel berhasil ditambahkan ke daftar favorite.');</script>";
        } else {
            echo "<script>alert('Terjadi kesalahan saat menambahkan artikel ke daftar favorite.');</script>";
        }
    }
}
...
// Ambil data dari GET, bukan POST
$selectedCategory = isset($_GET['category']) ? intval($_GET['category']) : 0;
$keyword = isset($_GET['keyword']) ? $_GET['keyword'] : '';

// Query ke tabel article dengan filter kategori jika dipilih
if ($selectedCategory > 0) {
    $sql = "SELECT a.* FROM article a
            JOIN article_category ac ON a.id = ac.article_id
            WHERE (a.title LIKE ? OR a.content LIKE ?) AND ac.category_id = ?";
    $stmt = $conn->prepare($sql);
    $search_param = "%" . $keyword . "%";
    $stmt->bind_param("ssi", $search_param, $search_param, $selectedCategory);
} else {
    $sql = "SELECT * FROM article WHERE title LIKE ? OR content LIKE ?";
    $stmt = $conn->prepare($sql);
    $search_param = "%" . $keyword . "%";
...
<head>
    ...
    <title>Bollyreads | Dashboard</title>
    ...
    <div class="flex flex-col md:flex-row lg:p-2 lg:ml-2 bg-white">
        ...
        <div class="p-4 flex-1 md:ml-64 ml-2 mr-1 rounded-lg bg-wattpad">
            ...
            <div class="flex mb-6 text-gray-800 relative rounded-t-lg">
                <!-- Form Pencarian -->
                <form method="POST" action="dashboard.php" class="flex-1 relative">
            ...
            <!-- Movie List -->
            <h2 class="text-xl mb-4 font-bold text-white">Daftar Artikel Film</h2>
            <?php if ($result->num_rows > 0): ?>
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2">
                    <?php while ($row = mysqli_fetch_assoc($result)) { ?>
                        <div class="card-wattpad rounded-md border border-white overflow-hidden shadow-lg">
                            <img src="./assets/artikel/<?php echo htmlspecialchars($row['image_url']); ?>" alt="Poster Film"
                            ...
                            <div class="p-4">
                                ...
                                <p class="text-sm mb-2 text-gray-800 truncate"
                                    title="<?php echo htmlspecialchars($row['publish_date']); ?>">
                                    Tanggal Rilis: <?php echo htmlspecialchars($row['publish_date']); ?>
                                </p>
                                <p class="text-xs text-gray-800 mb-2">
                                    <?php echo mb_strimwidth(strip_tags($row['content']), 0, 80, '...'); ?>
                                </p>
                                ...
                                <div class="flex items-center gap-x-2 text-xs">
                                    <a href="detail_article.php?id=<?php echo $row['id']; ?>"
                                        class="btn-uniform bg-orange-500 hover:bg-orange-600 text-white rounded flex items-center justify-center gap-1">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="w-4 h-4"
                                            viewBox="0 0 16 16">
                                            <path
    ...
    <script>
        ...
        document.querySelector('form').addEventListener('submit', function (e) {
            if (e.target.querySelector('#search-input')) {
                ...
                // Submit form dengan keyword dan kategori
